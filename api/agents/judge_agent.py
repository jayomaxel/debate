...
1. 逻辑性（logic_score）：论证是否严密、推理是否合理
2. 论据质量（argument_score）：论据是否充分、有说服力
3. 反应速度（response_score）：回应是否及时、切中要害
4. 说服力（persuasion_score）：语言是否有感染力、能否打动人
5. 团队配合（teamwork_score）：是否与队友配合良好
...
评分情况：
- 逻辑性：{score.logic_score}分
- 论据质量：{score.argument_score}分
- 反应速度：{score.response_score}分
- 说服力：{score.persuasion_score}分
- 团队配合：{score.teamwork_score}分
