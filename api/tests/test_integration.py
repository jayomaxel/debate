...
from middleware.rate_limit import reset_rate_limit_state
...
def _use_test_database(db_session):
    app.dependency_overrides[get_db] = lambda: db_session
    reset_rate_limit_state()

@pytest.mark.asyncio

async def test_complete_user_flow(db_session):
    """
    测试完整的用户流程：
    1. 教师注册和登录
    2. 创建班级
...
@pytest.mark.asyncio
async def test_authentication_flow(db_session):
    """测试认证流程"""
    _use_test_database(db_session)
    async with AsyncClient(transport=ASGITransport(app=app), base_url=BASE_URL) as client:
        # 注册
...
@pytest.mark.asyncio
async def test_permission_control(db_session):
    """测试权限控制"""
    _use_test_database(db_session)
    async with AsyncClient(transport=ASGITransport(app=app), base_url=BASE_URL) as client:
        # 创建教师和学生
